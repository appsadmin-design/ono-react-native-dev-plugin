...
5. Decompose the DD's Technical Implementation Approach (§19) and Impacted Modules (§20) into `templates/task-breakdown-template.md` rows — each task small enough for a single `/implement-task` run, each with an explicit `platform` value, `depends-on` dependencies, and explicit acceptance criteria sourced from the DD's Acceptance Criteria Mapping (§25).
