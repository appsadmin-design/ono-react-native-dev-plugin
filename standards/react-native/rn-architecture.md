...
- Where a repo's existing structure predates these principles, `repo-analyst`'s detected conventions take precedence for that repo until a migration is planned — see `standards/react-native/react-navigation.md` for the same repo-detection-first approach applied to navigation.
