...
These standards apply to any screen or component reviewed by whichever platform's code-reviewer agent is active (`rn-code-reviewer`, `ios-code-reviewer`, `android-code-reviewer`, `react-code-reviewer`), and are used by the matching platform feature-developer agent during implementation. They loosely follow WCAG 2.1 and the platform accessibility guidelines from Apple and Google; see [References](#references). Today's rule text uses React Native prop names (`accessibilityRole`, `accessibilityLabel`, etc.) — iOS (UIKit/SwiftUI accessibility APIs), Android (Compose/View accessibility APIs), and React web (ARIA) equivalents are not yet authored, see the `standards/ios/`, `standards/android/`, `standards/react/` stubs. Each bullet carries a stable ID so review findings can cite the exact rule they violate. This list is a baseline, not exhaustive — reviewers should use judgment for app-specific risk.
