...
These standards apply to mobile app code reviewed by the shared `mobile-security-reviewer` agent via `/review-security`. Today's rules are written from React Native experience (JS/TS application code, native bridge modules, build/manifest configuration); iOS- and Android-native-specific concerns, and web-specific concerns (XSS/CSRF/CSP) for the React platform, are not yet covered here — see `standards/ios/`, `standards/android/`, and `standards/react/` stubs. Categories loosely follow the OWASP Mobile Application Security Verification Standard (MASVS); see [References](#references). Each bullet below carries a stable ID so review findings can cite the exact rule they violate. This list is a baseline, not exhaustive — reviewers should use judgment for app-specific risk.
