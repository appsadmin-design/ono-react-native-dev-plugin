...
These standards apply to any screen, component, or copy that reaches an end user, reviewed by whichever platform's code-reviewer agent is active (`rn-code-reviewer`, `ios-code-reviewer`, `android-code-reviewer`, `react-code-reviewer`) via `/review-code`. The bidirectional-testing and locale-aware-formatting principles below are platform-agnostic; the "react-i18next Conventions" section is React Native-specific today — iOS/Android/React equivalents (NSLocalizedString/String Catalogs, Android string resources, i18next-for-web or similar) are not yet authored, see the `standards/ios/`, `standards/android/`, `standards/react/` stubs. Each bullet below carries a stable ID so review findings can cite the exact rule they violate. This list is a baseline, not exhaustive — reviewers should use judgment for app-specific risk.
